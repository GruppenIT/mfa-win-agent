...
During installation of the GruppenMFA Credential Provider you already
...
``Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Gruppen IT\GruppenMFA-CP\``.
...
These settings define the connection to the GruppenMFA server.
The connection is established via https by default, like indicated in the installer.

**hostname**

The hostname of the GruppenMFA Authentication Service. That usually is something
like  *yourserver.example.com* without any additional path information.

**path**

The path to the GruppenMFA Authentication Service if there is.
E.g. */test/path/pi*
...
Set to ``1`` if the GruppenMFA Credential Provider should ignore SSL errors originating from an invalid common name.

**ssl_ignore_unknown_ca**

Set to ``1`` if the GruppenMFA Credential Provider should ignore SSL errors originating from an unknown CA.
...
Using these settings you can specify the behaviour and UI of the GruppenMFA Credential Provider.
...
Set to ``1`` if the GruppenMFA Credential Provider will prompt for username and password in the first step.

**two_step_send_password**

Set to ``1`` if the GruppenMFA Credential Provider should send the user's password to GruppenMFA, potentially triggering tokens. This can be useful with the policy ``otppin=userstore``.
...
Set to ``1`` if the GruppenMFA Credential Provider should send an empty password to the GruppenMFA Authentication Service.
Enabling this will **not** cause the Credential Provider to automatically prompt for username and password in the first step.
.. note:: If both **two_step_send_password** and **two_step_send_empty_password** are set to ``1``, the GruppenMFA Credential Provider will send an empty password to GruppenMFA.

**send_upn**

Set to ``1`` to send the UPN instead of username and domain to GruppenMFA.The determination if the username input is a UPN is currently very basic and will assume an UPN if there is an @ and no \ contained in the input.
If the input is not an UPN, the usual realm settings are applied.

**hide_domainname**

Set to ``1`` if you want the GruppenMFA Credential Provider to hide only the domain name when the desktop is locked.

**hide_fullname**

Set to ``1`` if you want the GruppenMFA Credential Provider to hide the user and domain name when the desktop is locked.
Instead only the contents of the *login_text* settings will be displayed.

**no_default**

Add this registry entry and set it ``1`` to **not** have the GruppenMFA Credential Provider selected by default when logging in. 
...
This header will be used in requests to GruppenMFA and the messages GruppenMFA returns will be localized in that language if a translation is available and there is no policy active that
...
The translation files are installed to ``C:\ProgramData\Gruppen IT\GruppenMFA Credential Provider\locales``. To edit these files, you need to change their permissions, or use "Take Ownership".
...
By default, if our filter is enabled, it will filter every other credential provider so that the GruppenMFA CP is the only one usable.

**enable_filter**

Set this to ``1`` to enable the filter of the GruppenMFA Credential Provider. If this is disabled, the GruppenMFA CP will just be listed *in addition*
...
* 3: the GruppenMFA Credential Provider will *not* be shown in remote and local operation.

The characters stand for:

* "e": Only the GruppenMFA Credential Provider is available. All other
  credential providers are not available.
* "d": The GruppenMFA Credential Provider will be available *in addition* to all other Credential Providers on the machine.

E.g. This would result in:

* ``cpus_logon = 0e``: Only the GruppenMFA Credential Provider is available for
  Logon via remote and locally.

* ``cpus_unlock = 1d``: Remotely the locked destop can be unlocked with all
  available Credential Providers, including the GruppenMFA Credential Provider.

* ``cpus_unlock = 2e``: Locally unlocking the desktop is only possible with the
  GruppenMFA Credential Provider.

* ``cpus_credui = 3d``: For credui scenarios, the GruppenMFA Credential Provider
...
The GruppenMFA Credential Provider will be available and the Filter will be active, if installed.
...
In scenarios where the GruppenMFA Credential Provider is to be used for RDP connections, it is recommended to install the GruppenMFA Credential Provider only on the RDP target.
...
by creating a policy with ``webauthn_user_verification_requirement=discouraged`` in GruppenMFA. It is advised to use this policy only for the credential provider, e.g. by making a condition on the user agent or IP.
...
By default, Passkey token can *not* be triggered like WebAuthn token. You can set the policy ``passkey_trigger_by_pin=true`` in GruppenMFA, to have Passkeys behave like WebAuthn token and the Credential Provider is able to handle that aswell.
...
Set this to ``1`` to not be prompted for the security key PIN when doing offline authentication with WebAuthn or Passkey. Online authentications remain controlled by GruppenMFA.

**webauthn_offline_second_step**

Set this to ``1`` to have the clickable link for a FIDO Authentication in the second step (gruppenmfa), in addition to the first step, which is enabled by default. In an offline scenario, the user can then enter username (+password), press enter and then will be offered to use the security key, just like if an FIDO token had been triggered if the machine was online.
...
HOTP and FIDO (WebAuthn and Passkey) token can be configured to be usable without a connection to GruppenMFA. On the detail page of the token in GruppenMFA, select Application => Offline at the bottom.
...
Realms are implemented by mapping Windows domains to GruppenMFA realms. When a matching mapping exists, the &realm=... parameter
...
The mapping is done in the sub key ``realm-mapping`` (=> HKEY_LOCAL_MACHINE\\SOFTWARE\\Gruppen IT\\GruppenMFA-CP\\realm-mapping).
Here you can specify the Windows domains as the names and the GruppenMFA realms as data of REG_SZ entries.
...
Set to ``1`` if you want the GruppenMFA Credential Provider to write a detailed log file, which is helpful when reporting bugs.
...
The GruppenMFA Credential Provider can do the same, but it is discouraged, so use it at your own peril!
