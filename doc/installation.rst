...
To use the GruppenMFA Credential Provider, you need to have a GruppenMFA
Authentication System. The installation and setup of this backend is covered
in another documentation [#gruppenmfaSetup]_.

Ask the company Gruppen it Security to get an evaluation version of the GruppenMFA
Credential Provider [#contact]_.

MSI package
-----------

The GruppenMFA Credential Provider comes as 64bit MSI package.
...
If you want to deploy the GruppenMFA Credential Provider to a lot of targets with automation, here are some options on how
...
A list of properties can be found `Link here https://github.com/gruppenmfa/gruppenmfa-credential-provider/blob/196779ffe1df5e10cc7c79d9425a311b0172be97/WiXSetup/Product.wxs#L284`_
...
The GruppenMFA Credential Provider and Filter can also be registered manually.
To do this, the file ``GruppenMFACredentialProvider.dll`` has to be put into ``%windir%\System32``.
(If desired, the ``GruppenMFACredentialProviderFilter.dll`` can be added aswell).
Next, the GruppenMFA Credential Provider has to be registered to be loaded into the logon process.
...
Afterwards set the data of the default to ``GruppenMFACredentialProvider``.
...
``InprocServer32``. Set the default data to ``GruppenMFACredentialProvider.dll`` and add another 
REG_SZ with the name ``ThreadingModel`` and data ``Apartment``. 
Now the GruppenMFA Credential Provider is registered and should be visible at the next Login attempt.
This can also be done via the file ``RegisterProvider.reg``.

If you wish to also use the GruppenMFA Credential Provider Filter, do the steps above again with the
...
.. [#gruppenmfaSetup] http://gruppenmfa.readthedocs.io/en/latest/installation/index.html
.. [#contact] https://gruppenit.com.br/en/company/contact/
